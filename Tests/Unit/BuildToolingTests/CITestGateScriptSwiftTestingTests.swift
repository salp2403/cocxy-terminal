...
struct CITestGateScriptSwiftTestingTests {
    ...
    func performanceWorkflowEnforcesBenchmarkRegressionBaselines() throws {
        ...
        #expect((ciMetric("syntax_viewport_capture_ms")?["baseline"] as? NSNumber)?.doubleValue == 5.0)
        #expect((ciMetric("syntax_viewport_capture_ms")?["absolute_tolerance"] as? NSNumber)?.doubleValue == 1.5)
