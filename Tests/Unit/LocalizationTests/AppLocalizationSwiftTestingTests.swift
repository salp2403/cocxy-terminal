...
struct AppLocalizationSwiftTestingTests {
    ...
    func localizerLoadsEnglishAndSpanishResources() throws {
        ...
        #expect(spanish.string("preferences.section.spotlight", fallback: "Spotlight") == "Spotlight")
        #expect(spanish.string("preferences.spotlight.enable", fallback: "Enable local Spotlight indexing") == "Activar indexado local en Spotlight")
        #expect(spanish.string("preferences.spotlight.ignore.caption", fallback: "Create .cocxy-spotlight-ignore in a workspace to block indexing for that project.") == "Crea .cocxy-spotlight-ignore en un espacio de trabajo para bloquear el indexado de ese proyecto.")
        ...
        #expect(spanish.string("preferences.section.security", fallback: "Security") == "Seguridad")
        #expect(spanish.string("sandboxInspector.grants.section", fallback: "Capability Grants") == "Permisos concedidos")
        #expect(spanish.string("sandboxInspector.revoke", fallback: "Revoke") == "Revocar")
        #expect(spanish.string("sandboxInspector.capability.network-client", fallback: "Network client") == "Cliente de red")
        ...
        #expect(spanish.string("preferences.about.localDiagnostics", fallback: "Local Diagnostics") == "Diagnóstico local")
        #expect(spanish.string("preferences.about.unavailable", fallback: "Unavailable") == "No disponible")
        #expect(spanish.string("preferences.about.localOnlyDiagnostics", fallback: "Displayed locally only. Never uploaded.") == "Se muestra solo localmente. Nunca se sube.")
    ...
    func tabBarAndAuroraSidebarStringsLocalizeSpanish() throws {
        ...
        #expect(HorizontalTabStripView.localizedRenamePanelTitle(using: spanish) == "Renombrar panel...")
