...
extension AppDelegate {
    ...
    @MainActor
    private func activeCocxyCoreSurfaceForCLI() -> (
        controller: MainWindowController,
        surfaceID: SurfaceID,
        cocxyBridge: CocxyCoreBridge
    )? {
        guard let (controller, surfaceID) = activeTerminalSurfaceForCLI(),
              let cocxyBridge = controller.cocxyCoreBridge(forSurface: surfaceID) else {
            return nil
        }
        return (controller, surfaceID, cocxyBridge)
    }
    ...
    func runtimeStatusDetailsForCLI() -> [String: String] {
        ...
        if let images = cocxyBridge.imageDiagnostics(for: surfaceID) {
            ...
            data["image_iterm2_enabled"] = images.iterm2Enabled ? "true" : "false"
            data["image_disk_cache_enabled"] = images.diskCacheEnabled ? "true" : "false"
            data["image_disk_cache_used_bytes"] = "\(images.diskCacheBytes)"
            data["image_disk_cache_used_mib"] = "\(images.diskCacheBytes / (1024 * 1024))"
            data["image_disk_cache_limit_bytes"] = "\(images.diskCacheLimitBytes)"
            data["image_disk_cache_limit_mib"] = "\(images.diskCacheLimitBytes / (1024 * 1024))"
    ...
    func resetTerminalForCLI() -> [String: String]? {
        guard let (_, surfaceID, cocxyBridge) = activeCocxyCoreSurfaceForCLI(),
    ...
    func commandBlocksForCLI(limit: UInt32) -> [String: String]? {
        guard let (controller, surfaceID, cocxyBridge) = activeCocxyCoreSurfaceForCLI() else {
            return nil
        }
    ...
    func commandBlockOutputsForCLI(limit: UInt32) -> [String: String]? {
        ...
        let output = TerminalBlockOutputContextFormatter.text(for: blocks)
        return [
            "limit": "\(limit)",
    ...
    func listImagesForCLI() -> [String: String]? {
        ...
        if let diagnostics = cocxyBridge.imageDiagnostics(for: surfaceID) {
            ...
            data["iterm2_enabled"] = diagnostics.iterm2Enabled ? "true" : "false"
            data["disk_cache_enabled"] = diagnostics.diskCacheEnabled ? "true" : "false"
            data["disk_cache_used_bytes"] = "\(diagnostics.diskCacheBytes)"
            data["disk_cache_limit_bytes"] = "\(diagnostics.diskCacheLimitBytes)"
